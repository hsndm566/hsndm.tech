...
type Instructions = { configured: boolean; bankName: string; accountName: string; iban: string; amountSar: number; reference: string };
...
function OrderPanel({ result }: { result: Result }) {
  ...
  return <section className="cv-payment-panel"><div className="cv-payment-heading"><span>EXPORT CHECKOUT</span><b>{order?.status === "approved" ? "UNLOCKED" : `${instructions?.amountSar || 29} SAR`}</b></div>
    ...
    {order && order.status !== "approved" && order.status !== "rejected" && <div className="cv-transfer-flow"><div className={`cv-order-status status-${order.status}`}><b>Order {order.id}</b><span>{order.status === "pending_transfer" ? "Waiting for transfer" : "Transfer submitted — awaiting approval"}</span></div>{instructions?.configured ? <div className="cv-bank-details"><b>Transfer to</b><span>{instructions.bankName}</span><span>{instructions.accountName}</span><code>{instructions.iban}</code><small>Amount: {instructions.amountSar} SAR · {instructions.reference}</small></div> : <div className="cv-bank-details cv-bank-missing"><b>Bank details are not configured yet.</b><span>The admin must add BANK_NAME, BANK_ACCOUNT_NAME, and BANK_IBAN to the backend before customers can transfer.</span></div>}<form onSubmit={submitTransfer} className="cv-payment-form"><label><span>Payer name</span><input required value={payerName} onChange={(event) => setPayerName(event.target.value)} placeholder="Name used for the transfer" /></label><label><span>Transfer reference</span><input required value={transferReference} onChange={(event) => setTransferReference(event.target.value)} placeholder={order.id} /></label><button className="button button-accent" disabled={busy || !instructions?.configured}>{busy ? "Submitting…" : "Submit transfer details"} <ArrowRight size={17} /></button></form></div>}
