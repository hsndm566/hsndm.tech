...
async function readPdfText(file: File): Promise<string> {
  ...
  if (isBrowser) pdfjs.GlobalWorkerOptions.workerSrc = new URL("pdfjs-dist/legacy/build/pdf.worker.min.mjs", import.meta.url).toString();
...
async function readDocxText(file: File): Promise<string> {
  // The UMD browser bundle exposes the same API but does not ship a declaration file.
